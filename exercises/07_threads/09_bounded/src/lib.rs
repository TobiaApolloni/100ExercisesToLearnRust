...
impl TicketStoreClient {
    pub fn insert(&self, draft: TicketDraft) -> Result<TicketId, TrySendError<Command>> {
        let (response_channel, response_receiver) = sync_channel(self.capacity);
        match self.sender.try_send(Command::Insert { draft, response_channel }) {
    ...
    pub fn get(&self, id: TicketId) -> Result<Option<Ticket>, TrySendError<Command>> {
        let (response_channel, response_receiver) = sync_channel(self.capacity);
        match self.sender.try_send(Command::Get { id, response_channel }) {
            Ok(_) => Ok(response_receiver.recv().unwrap()),
            Err(error) => Err(error),
        } 
    }
