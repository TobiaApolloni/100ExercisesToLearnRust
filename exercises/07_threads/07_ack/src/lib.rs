use std::sync::{mpsc::{Receiver, Sender}, BarrierWaitResult};
    ...
    loop {
        match receiver.recv() {
            Ok(Command::Insert { draft, response_sender}) => {
                if response_sender.send(store.add_ticket(draft)).is_err() {
                    break;
                }
            }
            Ok(Command::Get { id, response_sender }) => {
                if response_sender.send(store.get(id).cloned()).is_err() {
                    break;
                }
            }
            Err(_) => {
